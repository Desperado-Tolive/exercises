...
s = "123456789"
s1 = s[:]
print(type(s1)) # type依然是str
print(s)
print(s1)
print(id(s))
print(id(s1))
print()         # s与s1的id相同，str字符串完整切片s[:]并没有进行任何操作
...
print(id(s1))
print(id(s2))
print()         # s1与s2的id不同，str字符串部分切片s[:6]是创建出要切片的部分，因此每次切片地址不同，是独立的
...
print(id(s1))
print(id(s2))
print()         # s[0]和s1的id相同，str字符串切片只有一个元素时并不是创建一个新地址，而是直接指向s的元素[0]，即id(s[:1])==id(s[0])
...
#1 str.capitalize() 将字符串首字母大写，其余小写，返回字符串
s = "i LOVE LWY"
print(s.capitalize())   # I love lwy

#2 str.title()      按照标题格式进行大小写转换（每个单词首字母大写），返回字符串
print(s.title())        # I Love Lwy

#3 str.upper()      将所有英文字符变为大写，返回字符串
s = "i love lwy"
print(s.upper())        # I LOVE LWY

#4 str.lower()      将所有英文字符变为小写，返回字符串
s = "I LOVE LWY"
print(s.lower())        # i love lwy

#5 str.swapcase()   大小写交互换，返回字符串
s = "I love lwy"
print(s.swapcase())     # i LOVE LWY
print()

#6 len(str)     计算字符串的字符个数,以后也可以用于计算元组列表等序列（个数≠长度）
s = "Study makes me happy"
s1 = "Study makes me 快乐happy"
print(len(s))   # 20
print(len(s1))  # 22

#7 str.count()  计算字符串中出现指定字符串的次数，返回整型
print(s.count("m",7))   # 1
print()

#8 str.find()   查找字符串中是否具有指定的字符串，返回第一次出现的位置，查找不到返回-1
#  str.rfind()  反向查找
print(s.find("a",3))    # 7
print(s.find("z",0))    # -1

#9 str.index()  查找字符串中是否具有指定的字符串，返回第一次出现的位置，查找不到直接报错
#  str.rindex() 反向查找
print(s.index("a",3))   # 7
...
#11 str.endswith()    检测字符串是否以指定的字符串结尾(可指定起始检测位置)，返回布尔值
print(s.endswith("y"))      # True
print(s.endswith("h"))      # False
print()

#12 str.is()    is开头的函数，返回值都是布尔值
s = "Study makes me happy"
...
#13 str.split()         使用指定的字符将字符串卷分离成多个字符串，返回列表
#   str.split()         反向切割，同样是按str中的字符顺序返回list，并非是反向返回的
...
#14 str.partition()     同样是切割，split可指定次数切割多次，partition只能切割一次且返回值包含切割符，返回元组
print(s.partition("*"))
print(type(s.partition("*")))
'''
在str中搜索分隔符，并返回它之前的部分，分隔符本身，以及后面的部分。如果没有找到分隔符，则返回str和两个空字符串。
'''


#14 str.splitlines()    使用\n，\r或者\r\n将字符串卷分离成多个字符串，返回列表
s= "日照香炉生紫烟\n遥看瀑布挂前川\r飞流直下三千尺\r\n疑是银河落九天。"
print(s.splitlines())   # ['日照香炉生紫烟', '遥看瀑布挂前川', '飞流直下三千尺', '疑是银河落九天。']
print(s.splitlines(-5)) # ['日照香炉生紫烟\n', '遥看瀑布挂前川\r', '飞流直下三千尺\r\n', '疑是银河落九天。']
'''
特别说明
str.splitlines() 如果不设参数，以'\n'、'\r'、'\r\n'进行分割，在行边界处断开，'\n'、'\r'、'\r\n'不包括在列表内，如果设了参数且参数为True，则将'\n'、'\r'、'\r\n'n也一并返回
'''

#15 str.join(iterable)  使用指定的字符串作为连接符将序列中的内容连接成新的字符串，返回字符串
s1 = "*"
print(s1.join(s))
...
#16 str.ljust()     将字符串靠左对齐，可指定字符串长度，可在空白处填充指定字符（默认填充空格），返回字符串
s = "对齐"
print(s.ljust(5,"l"))   # 对齐lll

#17 str.center()    将字符串居中对齐，可指定字符串长度，可在空白处填充指定字符（默认填充空格），返回字符串
print(s.center(5,"c"))  # cc对齐c

#18 str.rjust()     将字符串靠右对齐，可指定字符串长度，可在空白处填充指定字符（默认填充空格），返回字符串
print(s.rjust(5,"r"))   # rrr对齐

#19 str.lstrip()    去除字符串左侧的指定字符串（直到新字符串最左侧不含指定字符串），默认去除空格，返回字符串
s = " abc去除"
print(s.lstrip("a"))# 从字符串开头第一个字符开始算，若指定字符串不是在字符串的起点，则不会去除任何字符
print(s.lstrip(" a"))   #  abc去除

#19 str.rstrip()    去除字符串右侧的指定字符串（直到新字符串最右侧不含指定字符串），默认去除空格，返回字符串
s = "去除abc "
print(s.rstrip("c"))# 从字符串结尾第一个字符开始算，若指定字符串不是在字符串的终点，则不会去除任何字符
print(s.rstrip("c "))   # 去除ab

#20 str.strip()     去除字符串两侧的指定字符串（直到新字符串两侧不含指定字符串），默认去除空格，返回字符串
s = "abcd去除fgi"
...
strip()是将指定字符串拆分成字符来执行，只要str两侧任何一侧（最左 和/或 最右）含有指定字符串中的任意一个字符，那么就会被去除，
也就是说，strip()的执行是不断循环检测str两侧是否存在指定字符串中的任意一个字符并去除，只有当两侧都不再存在相应字符，strip()才会终止，返回新字符串
str = "abcdefg"     str.strip("abdfg")的返回值是"cde"
'''

#21 str.zfill() # 将字符串靠右对齐，可指定字符串长度，在空白处填充0，返回字符串
s = "零填充"
print(s.zfill(9))   # 000000零填充

#22 str.replace(被替换字符串,替换字符串)   # 字符串替换操作
s = "我要睡。觉了。。 "
s1 = "。要。要。要 "
...
#23 str.maketrans('被替换的字符串','生成一个用于字符串替换的映射表，为了给translate函数使用，返回映射表
#24 str.translate(str.maketrans) 进行字符串翻译操作，类似转换
...
'''
"E:\Python workspace\PycharmProjects\习题课\venv\Scripts\python.exe" "E:/Python workspace/PycharmProjects/习题课/07-str.py"
<class 'str'>
123456789
123456789
2418761296432
