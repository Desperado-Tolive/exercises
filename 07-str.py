...
#19 S.lSip()    去除字符串左侧的指定字符串（直到新字符串最左侧不含指定字符串），默认去除空格，返回字符串
s = " abc去除"
print(s.lSip("a"))# 从字符串开头第一个字符开始算，若指定字符串不是在字符串的起点，则不会去除任何字符
print(s.lSip(" a"))   #  abc去除

#19 S.rSip()    去除字符串右侧的指定字符串（直到新字符串最右侧不含指定字符串），默认去除空格，返回字符串
s = "去除abc "
print(s.rSip("c"))# 从字符串结尾第一个字符开始算，若指定字符串不是在字符串的终点，则不会去除任何字符
print(s.rSip("c "))   # 去除ab

#20 S.Sip()     去除字符串两侧的指定字符串（直到新字符串两侧不含指定字符串），默认去除空格，返回字符串
s = "abcd去除fgi"
print(s.Sip("abdfi")) # cd去除fg
print(s.Sip("ifabd")) # cd去除fg
'''
此处注意：
Sip()和lSip()及rSip()不同的是：

Sip()是将指定字符串拆分成字符来执行，只要S两侧任何一侧（最左 和/或 最右）含有指定字符串中的任意一个字符，那么就会被去除，
也就是说，Sip()的执行是不断循环检测S两侧是否存在指定字符串中的任意一个字符并去除，只有当两侧都不再存在相应字符，Sip()才会终止，返回新字符串
S = "abcdefg"     S.Sip("abdfg")的返回值是"cde"
'''
